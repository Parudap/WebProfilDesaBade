...
<head>
    ...
    <title>Data Penduduk - Admin Desa Bade</title>
    ...
    <style>h1, h2, h3, h4, h5, h6 { font-family: 'Cinzel', serif; } 
        *{box-sizing:border-box}body{font-family:'Plus Jakarta Sans',sans-serif;background:#f4f6fb;color:#1e293b;margin:0}
        ...
        .tab-btn{padding:7px 16px;border-radius:8px;font-size:13px;font-weight:600;color:#64748b;background:transparent;border:none;cursor:pointer;transition:all .2s;font-family:inherit}
        ...
        .btn-add{display:inline-flex;align-items:center;gap:8px;padding:9px 18px;background:linear-gradient(135deg,#2e7d32,#1b5e20);color:white;border-radius:10px;font-size:13px;font-weight:600;border:none;cursor:pointer;transition:all .2s;font-family:inherit;box-shadow:0 3px 10px rgba(46,125,50,.3)}
        .btn-add:hover{transform:translateY(-1px);box-shadow:0 5px 16px rgba(46,125,50,.4)}
        .btn-edit{padding:5px 11px;border-radius:7px;font-size:12px;font-weight:600;color:#2e7d32;background:#f0fdf4;border:1px solid #bbf7d0;cursor:pointer;transition:all .15s;font-family:inherit}
        .btn-edit:hover{background:#dcfce7}
        .btn-del{padding:5px 11px;border-radius:7px;font-size:12px;font-weight:600;color:#ef4444;background:#fef2f2;border:1px solid #fecaca;cursor:pointer;transition:all .15s;font-family:inherit}
        .btn-del:hover{background:#fee2e2}
        .val-num{font-weight:700;color:#2e7d32}
        .val-total{font-weight:800;color:#1e293b}
        ...
        .modal{background:#fff;border-radius:18px;width:460px;max-width:95vw;box-shadow:0 24px 60px rgba(0,0,0,.18);transform:translateY(16px) scale(.97);transition:all .25s}
        ...
        .form-input,.form-select{width:100%;padding:9px 13px;border:1px solid #e2e8f0;border-radius:9px;font-size:13px;font-family:inherit;color:#1e293b;background:#fff;transition:all .2s}
        .form-input:focus,.form-select:focus{outline:none;border-color:#2e7d32;box-shadow:0 0 0 3px rgba(46,125,50,.1)}
        .btn-submit{padding:10px 22px;background:linear-gradient(135deg,#2e7d32,#1b5e20);color:white;border-radius:9px;font-size:13px;font-weight:600;border:none;cursor:pointer;font-family:inherit;transition:all .2s}
    ...
    <nav class="sidebar-nav">
        ...
        <div class="nav-group">
            <div class="nav-group-header" onclick="toggleNav('infografis',this)">
                ...
                <svg id="infografis-icon" class="nav-group-chevron open" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
            </div>
            <div id="infografis" class="nav-submenu open">
                <a href="{{ route('admin.infografis.penduduk') }}" class="nav-subitem active-sub">Penduduk</a>
...
<div class="main">
    <header class="topbar">
        <div class="topbar-left"><h2>Data Penduduk</h2><p>Kelola statistik kependudukan Desa Bade</p></div>
        <div class="breadcrumb">
            <a href="{{ route('admin.dashboard') }}">Dashboard</a><span>&rsaquo;</span>
            <span>Infografis</span><span>&rsaquo;</span>
            <span style="color:#2e7d32;font-weight:600;">Penduduk</span>
        </div>
    </header>
    ...
    <main class="page-content">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
            <div class="tabs" id="kategori-tabs">
                @foreach($tabs as [$k,$l])
                <button class="tab-btn {{ $k === $activeTab ? 'active' : '' }}" onclick="switchKategori('{{ $k }}',this)">
                    {{ $l }} <span style="font-size:11px;background:#f0fdf4;color:#2e7d32;border-radius:20px;padding:2px 7px;margin-left:4px;font-weight:700;">{{ $k === 'dusun' ? $dusunList->count() : $data[$k]->count() }}</span>
                </button>
                @endforeach
            </div>
        ...
        @foreach($tabs as [$k,$l])
        @if($k !== 'dusun')
        <div id="panel-{{ $k }}" class="card" style="{{ $k !== $activeTab ? 'display:none' : '' }}">
            <div class="table-wrap" style="overflow-x:auto;">
                <table>
                    ...
                    <tbody>
                        @forelse($data[$k] as $i => $item)
                        <tr>
                            <td class="td-no">{{ $i+1 }}</td>
                            <td style="font-weight:600">{{ $item->label }}</td>
                            <td style="text-align:right" class="val-num">{{ number_format($item->value_laki) }}</td>
                            <td style="text-align:right" class="val-num">{{ number_format($item->value_perempuan) }}</td>
                            <td style="text-align:right" class="val-total">{{ number_format($item->value_laki + $item->value_perempuan) }}</td>
                        ...
                        @empty
                        <tr class="empty-row"><td colspan="7">Belum ada data {{ $l }}. Klik <strong>Tambah Data</strong>.</td></tr>
                        @endforelse
                        @if($data[$k]->count())
                        <tr style="background:#f8fafc;border-top:2px solid #e2e8f0;">
                            <td colspan="2" style="font-weight:700;font-size:13px;padding:12px 16px;">Total</td>
                            <td style="text-align:right;font-weight:800;color:#2e7d32;padding:12px 16px;">{{ number_format($data[$k]->sum('value_laki')) }}</td>
                            <td style="text-align:right;font-weight:800;color:#2e7d32;padding:12px 16px;">{{ number_format($data[$k]->sum('value_perempuan')) }}</td>
                            <td style="text-align:right;font-weight:800;color:#1e293b;padding:12px 16px;">{{ number_format($data[$k]->sum('value_laki') + $data[$k]->sum('value_perempuan')) }}</td>
                            <td colspan="2"></td>
                        </tr>
                        @endif
                    </tbody>
        ...
        @else
        {{-- Panel Dusun --}}
        <div id="panel-dusun" class="card" style="{{ $activeTab !== 'dusun' ? 'display:none' : '' }}">
            <div class="table-wrap" style="overflow-x:auto;">
                <table>
                    <thead><tr>
                        <th class="td-no">No</th>
                        <th>Nama Dusun</th>
                        <th style="text-align:right">Jumlah KK</th>
                        <th style="text-align:right">Laki-laki</th>
                        <th style="text-align:right">Perempuan</th>
                        <th style="text-align:right">Total Jiwa</th>
                        <th style="text-align:center">Urutan</th>
                        <th class="td-actions">Aksi</th>
                    </tr></thead>
                    <tbody>
                        @forelse($dusunList as $i => $item)
                        <tr>
                            <td class="td-no">{{ $i+1 }}</td>
                            <td style="font-weight:700;color:#1e293b">{{ $item->nama }}</td>
                            <td style="text-align:right;font-weight:600" class="val-num">{{ number_format($item->kk) }} KK</td>
                            <td style="text-align:right" class="val-num">{{ number_format($item->laki) }}</td>
                            <td style="text-align:right" class="val-num">{{ number_format($item->perempuan) }}</td>
                            <td style="text-align:right;font-weight:800;color:#2e7d32" class="val-total">{{ number_format($item->jiwa) }} Jiwa</td>
                            <td style="text-align:center;color:#94a3b8">{{ $item->urutan }}</td>
                            <td class="td-actions">
                        ...
                        @empty
                        <tr class="empty-row"><td colspan="8">Belum ada data Dusun. Klik <strong>Tambah Data</strong>.</td></tr>
                        @endforelse
                        @if($dusunList->count())
                        <tr style="background:#f8fafc;border-top:2px solid #e2e8f0;">
                            <td colspan="2" style="font-weight:700;font-size:13px;padding:12px 16px;">Total Keseluruhan</td>
                            <td style="text-align:right;font-weight:800;color:#1e293b;padding:12px 16px;">{{ number_format($dusunList->sum('kk')) }} KK</td>
                            <td style="text-align:right;font-weight:800;color:#2e7d32;padding:12px 16px;">{{ number_format($dusunList->sum('laki')) }}</td>
                            <td style="text-align:right;font-weight:800;color:#2e7d32;padding:12px 16px;">{{ number_format($dusunList->sum('perempuan')) }}</td>
                            <td style="text-align:right;font-weight:800;color:#2e7d32;padding:12px 16px;">{{ number_format($dusunList->sum('jiwa')) }} Jiwa</td>
                            <td colspan="2"></td>
                        </tr>
                        @endif
                    </tbody>
...
<div class="modal-backdrop" id="modal-add">
    <div class="modal">
        ...
        <form method="POST" action="{{ route('admin.infografis.penduduk.store') }}">@csrf
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label class="form-label">Label / Kelompok <span class="req">*</span></label>
                    <input type="text" name="label" class="form-input" placeholder="cth: 20-24" required maxlength="100">
                </div>
                <div class="form-row">
                    <div class="form-group" style="margin-bottom:0">
                        <label class="form-label">Laki-laki <span class="req">*</span></label>
                        <input type="number" name="value_laki" class="form-input" min="0" value="0" required>
                    </div>
                    <div class="form-group" style="margin-bottom:0">
                        <label class="form-label">Perempuan <span class="req">*</span></label>
                        <input type="number" name="value_perempuan" class="form-input" min="0" value="0" required>
                    </div>
                </div>
                <div class="form-group" style="margin-top:16px;margin-bottom:0">
                    <label class="form-label">Urutan</label>
                    <input type="number" name="urutan" class="form-input" min="0" value="0">
                </div>
            </div>
            <div class="modal-footer"><button type="button" class="btn-muted" onclick="closeModal('modal-add')">Batal</button><button type="submit" class="btn-submit">Simpan</button></div>
        </form>
    </div>
</div>

<!-- MODAL TAMBAH DUSUN -->
<div class="modal-backdrop" id="modal-add-dusun">
    <div class="modal">
        <div class="modal-header">
            <span class="modal-title">
                <svg width="18" height="18" fill="none" stroke="#2e7d32" viewBox="0 0 24 24">
            ...
            <button class="modal-close" onclick="closeModal('modal-add-dusun')">&times;</button>
        </div>
        <form method="POST" action="{{ route('admin.infografis.dusun.store') }}">@csrf
            <div class="modal-body">
                <div class="form-group">
                    <label class="form-label">Nama Dusun <span class="req">*</span></label>
                    <input type="text" name="nama" class="form-input" placeholder="cth: Pelang" required maxlength="100">
                </div>
                <div class="form-group">
                    <label class="form-label">Jumlah KK (Kartu Keluarga) <span class="req">*</span></label>
                    <input type="number" name="kk" class="form-input" min="0" value="0" required>
                </div>
                <div class="form-row">
                    <div class="form-group" style="margin-bottom:0">
                        <label class="form-label">Jumlah Laki-laki <span class="req">*</span></label>
                        <input type="number" name="laki" class="form-input" min="0" value="0" required>
                    </div>
                    <div class="form-group" style="margin-bottom:0">
                        <label class="form-label">Jumlah Perempuan <span class="req">*</span></label>
                        <input type="number" name="perempuan" class="form-input" min="0" value="0" required>
                    </div>
                </div>
                <div class="form-group" style="margin-top:16px;margin-bottom:0">
            ...
            <div class="modal-footer"><button type="button" class="btn-muted" onclick="closeModal('modal-add-dusun')">Batal</button><button type="submit" class="btn-submit">Simpan</button></div>
        </form>
    </div>
</div>
...
<div class="modal-backdrop" id="modal-edit">
    <div class="modal">
        ...
        <form method="POST" id="form-edit" action="">@csrf @method('PUT')
            <div class="modal-body">
                <div class="form-group">
                    <label class="form-label">Kategori</label>
                    <input type="text" id="edit-kategori-display" class="form-input" disabled style="background:#f8fafc">
                ...
                <div class="form-row">
                    <div class="form-group" style="margin-bottom:0">
                        <label class="form-label">Laki-laki <span class="req">*</span></label>
                        <input type="number" name="value_laki" id="edit-laki" class="form-input" min="0" required>
                    </div>
                    <div class="form-group" style="margin-bottom:0">
                        <label class="form-label">Perempuan <span class="req">*</span></label>
                        <input type="number" name="value_perempuan" id="edit-perempuan" class="form-input" min="0" required>
                    </div>
                </div>
                <div class="form-group" style="margin-top:16px;margin-bottom:0">
                    <label class="form-label">Urutan</label>
            ...
            <div class="modal-footer"><button type="button" class="btn-muted" onclick="closeModal('modal-edit')">Batal</button><button type="submit" class="btn-submit">Simpan</button></div>
        </form>
    </div>
</div>

<!-- MODAL EDIT DUSUN -->
<div class="modal-backdrop" id="modal-edit-dusun">
    <div class="modal">
        <div class="modal-header">
            <span class="modal-title">
                <svg width="18" height="18" fill="none" stroke="#2e7d32" viewBox="0 0 24 24">
            ...
            <button class="modal-close" onclick="closeModal('modal-edit-dusun')">&times;</button>
        </div>
        <form method="POST" id="form-edit-dusun" action="">@csrf @method('PUT')
            <div class="modal-body">
                <div class="form-group">
                    <label class="form-label">Nama Dusun <span class="req">*</span></label>
                    <input type="text" name="nama" id="edit-dusun-nama" class="form-input" required maxlength="100">
                </div>
                <div class="form-group">
                    <label class="form-label">Jumlah KK (Kartu Keluarga) <span class="req">*</span></label>
                    <input type="number" name="kk" id="edit-dusun-kk" class="form-input" min="0" required>
                </div>
                <div class="form-row">
                    <div class="form-group" style="margin-bottom:0">
                        <label class="form-label">Laki-laki <span class="req">*</span></label>
                        <input type="number" name="laki" id="edit-dusun-laki" class="form-input" min="0" required>
                    </div>
                    <div class="form-group" style="margin-bottom:0">
                        <label class="form-label">Perempuan <span class="req">*</span></label>
                        <input type="number" name="perempuan" id="edit-dusun-perempuan" class="form-input" min="0" required>
                    </div>
                </div>
                <div class="form-group" style="margin-top:16px;margin-bottom:0">
                    <label class="form-label">Urutan</label>
                    <input type="number" name="urutan" id="edit-dusun-urutan" class="form-input" min="0">
                </div>
            </div>
            <div class="modal-footer"><button type="button" class="btn-muted" onclick="closeModal('modal-edit-dusun')">Batal</button><button type="submit" class="btn-submit">Simpan</button></div>
        </form>
    </div>
</div>
...
<div class="modal-backdrop" id="modal-delete">
    <div class="modal">
        <div class="modal-body" style="padding-top:28px;">
            ...
            <div class="confirm-text"><h3>Hapus Data?</h3><p>Data <strong id="delete-name"></strong> akan dihapus permanen.</p></div>
        </div>
...
function switchKategori(k, btn) {
    ...
    document.querySelectorAll('#kategori-tabs .tab-btn').forEach(b => b.classList.remove('active'));
...
function openAddModal(){
    if (activeKategori === 'dusun') {
        openModal('modal-add-dusun');
    } else {
        if(document.getElementById('add-kategori')) document.getElementById('add-kategori').value = activeKategori;
        openModal('modal-add');
    }
}
function openEditModal(id,label,kategori,laki,perempuan,urutan){
    document.getElementById('form-edit').action=`/admin/infografis/penduduk/${id}`;
    document.getElementById('edit-label').value=label;
    document.getElementById('edit-kategori').value=kategori;
    document.getElementById('edit-kategori-display').value=kategori;
    document.getElementById('edit-laki').value=laki;
    document.getElementById('edit-perempuan').value=perempuan;
    document.getElementById('edit-urutan').value=urutan;
    openModal('modal-edit');
}
function openEditDusunModal(id,nama,kk,laki,perempuan,urutan){
    document.getElementById('form-edit-dusun').action=`/admin/infografis/dusun/${id}`;
    document.getElementById('edit-dusun-nama').value=nama;
    document.getElementById('edit-dusun-kk').value=kk;
    document.getElementById('edit-dusun-laki').value=laki;
    document.getElementById('edit-dusun-perempuan').value=perempuan;
    document.getElementById('edit-dusun-urutan').value=urutan;
    openModal('modal-edit-dusun');
}
function openDeleteModal(id,name){
    document.getElementById('form-delete').action=`/admin/infografis/penduduk/${id}`;
    document.getElementById('delete-name').textContent=name;
    openModal('modal-delete');
}
function openDeleteDusunModal(id,name){
    document.getElementById('form-delete').action=`/admin/infografis/dusun/${id}`;
    document.getElementById('delete-name').textContent=name;
    openModal('modal-delete');
}
</script>
