...
@section('content')
    <section id="berita-detail" class="section-pad">
        <div class="container-shell">
            ...
            <nav class="flex text-sm text-gray-500 mb-6 items-center gap-2" aria-label="Breadcrumb">
                ...
                <a href="/berita" class="hover:text-green-600 font-semibold transition">Berita Desa</a>
            </nav>

            <div class="grid gap-8 lg:grid-cols-3">
                <!-- Main Content (2 Columns) -->
                <div class="lg:col-span-2 content-card p-6 md:p-10">
                    <!-- Title -->
                    <h1 class="text-2xl md:text-3xl lg:text-4xl font-extrabold text-gray-950 leading-tight mb-5">
                        {{ $article['title'] }}
                    </h1>

                    <!-- Meta info -->
                    <div class="flex flex-wrap items-center gap-y-3 gap-x-6 text-xs text-gray-500 border-b border-gray-100 pb-5 mb-8">
                        <div class="flex items-center gap-1.5 font-semibold text-gray-600 whitespace-nowrap">
                            <svg class="h-4 w-4 text-green-600 flex-shrink-0 stroke-[2.2]" width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Banner Image & Gallery Slider -->
                    <div x-data="{ 
                        images: {{ json_encode(array_values(isset($article['images']) && count($article['images']) > 0 ? $article['images'] : [$article['image']])) }},
                        currentIndex: 0,
                        showLightbox: false,
                        next() { this.currentIndex = (this.currentIndex + 1) % this.images.length },
                        prev() { this.currentIndex = (this.currentIndex - 1 + this.images.length) % this.images.length }
                    }" class="mb-8">
                        <!-- Main Banner Container (Adaptive uncropped) -->
                        <div class="relative rounded-3xl overflow-hidden shadow-sm border border-gray-100 bg-gray-950/5 flex items-center justify-center min-h-[300px] max-h-[560px] group cursor-pointer" @click="showLightbox = true">
                            ...
                            <img :src="images[currentIndex]" alt="{{ $article['title'] }}" class="relative max-w-full max-h-[560px] w-auto h-auto object-contain transition duration-300 group-hover:scale-[1.01]">
                            ...
                            <div class="mt-4 flex flex-wrap gap-3">
                                <template x-for="(imgUrl, idx) in images" :key="idx">
                                    <div @click="currentIndex = idx"
                                        ...
                                        <img :src="imgUrl" alt="{{ $article['title'] }}" class="h-full w-full object-cover">
                                    </div>
                                </template>
                            </div>
                        </template>                        <!-- Lightbox Fullscreen Modal (Teleported to Body to avoid container transform clipping & overlay header) -->
                        <template x-teleport="body">
                            <div x-show="showLightbox" x-transition.opacity class="fixed inset-0 bg-black/95 backdrop-blur-md flex items-center justify-center p-4 md:p-8" style="position: fixed; inset: 0; z-index: 999999;" @click.self="showLightbox = false" x-cloak>
                                ...
                                <img :src="images[currentIndex]" alt="{{ $article['title'] }}" class="max-w-[92vw] max-h-[88vh] object-contain rounded-2xl shadow-2xl">
                    ...
                    <!-- Article Body -->
                    <div class="prose max-w-none text-gray-700 leading-relaxed space-y-5 text-[15px] antialiased">
                        {!! $article['content'] !!}
                    </div>

                    <!-- Sharing -->
                    <div class="mt-10 pt-6 border-t border-gray-100 flex items-center gap-3 flex-wrap" x-data="{
                        shareUrl: window.location.href,
                        shareTitle: '{{ addslashes($article['title']) }}',
                        copied: false,
                        copyLink() {
                        ...
                        <!-- Facebook -->
                        <a :href="'https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent(shareUrl)" target="_blank" rel="noopener noreferrer" class="h-9 w-9 rounded-full border border-blue-200 bg-white hover:bg-blue-50 text-[#1877f2] flex items-center justify-center transition shadow-sm" title="Bagikan ke Facebook">
                            <svg class="h-4 w-4 fill-current flex-shrink-0" width="16" height="16" viewBox="0 0 24 24">
                                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                            </svg>
                        </a>
                        <!-- Whatsapp -->
                        <a :href="'https://api.whatsapp.com/send?text=' + encodeURIComponent(shareTitle + ' ' + shareUrl)" target="_blank" rel="noopener noreferrer" class="h-9 w-9 rounded-full bg-[#25d366] hover:bg-opacity-90 text-white flex items-center justify-center transition shadow-sm" title="Bagikan ke WhatsApp">
                            <svg class="h-4 w-4 fill-current flex-shrink-0" width="16" height="16" viewBox="0 0 24 24">
                        ...
                        <!-- Twitter/X -->
                        <a :href="'https://twitter.com/intent/tweet?url=' + encodeURIComponent(shareUrl) + '&text=' + encodeURIComponent(shareTitle)" target="_blank" rel="noopener noreferrer" class="h-9 w-9 rounded-full bg-black hover:bg-opacity-90 text-white flex items-center justify-center transition shadow-sm" title="Bagikan ke X">
                            <svg class="h-4 w-4 fill-current flex-shrink-0" width="16" height="16" viewBox="0 0 24 24">
                        ...
                        <div class="relative flex items-center">
                            <button @click="copyLink()" class="h-9 w-9 rounded-full bg-gray-900 hover:bg-gray-800 text-white flex items-center justify-center transition shadow-sm" title="Salin Tautan">
                ...
                <div>
                    <div class="content-card p-6 sticky top-24">
                        <h3 class="text-base font-bold text-gray-900 border-b border-gray-100 pb-3.5 mb-5 flex items-center gap-2">
                            <span class="h-2 w-2 rounded-full bg-green-600"></span>
                            Berita Terbaru
                        </h3>
                        <div class="flex flex-col gap-5">
                            @foreach ($recentNews as $item)
                                <a href="{{ route('berita.show', $item['slug']) }}" class="flex gap-3 group items-start border-b border-gray-50 pb-5 last:border-0 last:pb-0">
                                    <div class="h-16 w-16 rounded-2xl overflow-hidden flex-shrink-0 border border-gray-100 bg-gray-50">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <h4 class="text-sm font-bold text-gray-900 leading-snug group-hover:text-green-600 transition duration-150 line-clamp-2">
                                            {{ $item['title'] }}
                                        </h4>
                                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] text-gray-400 mt-2 font-semibold">
                                            <span class="flex items-center gap-1 whitespace-nowrap">
                                                <svg class="h-3.5 w-3.5 text-gray-400 flex-shrink-0 stroke-[2]" width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                                </svg>
